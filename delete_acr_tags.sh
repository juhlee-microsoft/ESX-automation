function delete_act_tags() {
    ...
    local timestamp, manifest, tag, limitdate, _timestamp, availability
    ...
    az acr repository list --name "$acr_name" --output table --subscription "$subscription_id" > "$acr_name"_delete_acr_target_names.txt

    # Removed the first 2 lines from the original output
    sed -i "1,2d" "$acr_name"_delete_acr_target_names.txt

    # Create the original copy for recovery purpose
    cp "$acr_name"_delete_acr_target_names.txt "$acr_name"_delete_acr_target_names.original
    ...
    do
        ...
        do
            ...
            # PR image build stays for 30 days.
            if [[ "$tag" == *"-PR-"* ]];
            then
                availability=30
            else
            ...
            if [[ $_timestamp < $limitdate ]]; then
                ...
                if [ $? ]; then
                    echo "Failed to remove $repoName@$manifest"
                else
        ...
        sed -i "1d" "$acr_name"_delete_acr_target_names.txt

    done < """$acr_name""_delete_acr_target_names.txt"
}
